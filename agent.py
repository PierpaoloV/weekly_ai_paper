...
def score_item(it, cfg):
    ...
    for b in cfg["search"]["boost_terms"]:
        if b.lower() in t.lower():
            score += 2.0
    v = (it.get("venue") or "").lower()
    if any(k in v for k in ["medical image analysis", "tmi", "miccai", "nature", "science", "plos", "ieee"]):
        score += 1.5
    if contains_any(t, cfg["search"]["block_terms"]):
        score -= 2.0
    score += 0.2  # slight freshness bias (weekly run)
    return score
...
def build_email_html(picks):
    ...
    for i, p in enumerate(picks, 1):
        ...
        url     = p.get("url")
        abst    = html.escape(p.get("abstract") or "")
        doi     = p.get("doi")
        ...
        <tr>
          <td style="padding:12px; border-bottom:1px solid #eee;">
            ...
            <div style="color:#333; font-size:14px; line-height:1.4;">{abst}</div>
          </td>
        </tr>
        """)
    ...
    <div style="font-family:system-ui,Segoe UI,Arial; max-width:820px; margin:auto;">
      ...
      <p style="color:#555;">Selezione degli ultimi 7 giorni (PubMed + arXiv), ordinati per rilevanza.</p>
    """
    footer = """
      <p style="color:#999; font-size:12px; margin-top:16px;">
        Preferenze o nuove keyword? Modifica <code>config.yaml</code>.
      </p>
    </div>
    """
